 ...
 'use strict';
  ...
  var methods = {
    ...
    _add: function(button) {
      ...
      var item = this.model.clone(this.opt.clone);

      if (this.opt.add && !methods._execute.call(this, this.opt.add, item)) {
        return false;
      }

      if (this.opt.excludes) {
    ...
    _clear: function(item) {
      ...
      if (typeof this.opt.clear === 'function') {
        items = $(item).find(':input').filter(this.opt.clear);
      } else {
        items = $(item).find(this.opt.clear);
      }
    ...
    _rename: function(field, index, attribute) {
      var
        value   = $(field).data(attribute) || field[attribute],
        partner = '$1' + index + '$2';
    ...
    _exclude: function(item) {
      if (typeof this.opt.excludes === 'function') {
        item.find(':input').filter(this.opt.excludes).remove();
      } else {
    ...
    _reorder: function() {
      ...
      $(that).find(that.opt.model).each(function(index) {
        $(this).find(':input').each(function() {
          var
            name = methods._rename.call(that, this, index, 'name'),
            id   = methods._rename.call(that, this, index, 'id');
